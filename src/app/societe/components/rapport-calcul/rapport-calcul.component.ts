...
export class RapportCalculComponent implements OnInit {
  ...
  ngOnInit(): void {
    const sParam = this.route.snapshot.paramMap.get('id');
    const rParam = this.route.snapshot.paramMap.get('rapportId');
    const currentSocieteId = sParam ? Number(sParam) : Number(sessionStorage.getItem('societeId'));
    const currentRapportId = rParam ? Number(rParam) : Number(sessionStorage.getItem('rapportId'));
