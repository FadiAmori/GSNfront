...
import { LigneCalculeeService, UpdateLigneCalculeeDto } from '../../services/ligne-calculee.service';
import { ExcelLigneCalculee } from '../../services/excel-ligne-calculee.model';
...
export class RapportTableauComponent implements OnInit {
  ...
  get backLink(): string[] {
    return ['/admin/societes', String(this.societeId() ?? ''), 'rapports'];
  }

  get importExcelLink(): (string | number)[] {
    return ['/admin/societes', this.societeId() ?? '', 'rapports', this.rapportId() ?? '', 'import-excel'];
  }
  ...
  ngOnInit(): void {
    ...
    const societeId = societeIdParam ? Number(societeIdParam) : null;
...
private loadCloneSources(societeId: number, currentRapportId: number): void {
  this.rapportService.getAll().subscribe({
    next: (all) => {
      const options = (all ?? [])
        .filter(r => 
          r.societeId === 1 &&          // ← limiter à la société id=1
          r.id && 
          r.id !== currentRapportId
        )
